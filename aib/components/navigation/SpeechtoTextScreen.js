...
const ObjectDetectionScreen = () => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      formData.append('image', blob, { name: 'uploaded_image.jpg' }); 

      const axiosResponse = await axios.post('http://${IP_ADDRESS}:8000/api/detect_object/', formData, {  
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      const detectedObjects = axiosResponse.data.detection_results; 
  ...
  return (
    <View style={styles.container}>
      <Button title="Speak" onPress={pickImage} />
            {selectedImageUri && (
                <Image source={{ uri: selectedImageUri }} style={styles.image} />
            )}
            <Button title="Text to Speech" onPress={uploadImage} />
