...
const OCRScreen = () => {
    ...
    useEffect(() => {
        Speech.speak('Document Reading screen, press and hold to record audio, say capture to take a picture and upload to upload it.');
    }, []);
    ...
    const uploadImage = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://${IP_ADDRESS}:8000/api/process_image/',   
                formData,
                {
                    headers: {
            ...
            const ocrData = response.data.ocr_results;
            ...
            if (extractedText) {
                ...
                Speech.speak('No text detected in response!');
            }
        } catch (err) {
